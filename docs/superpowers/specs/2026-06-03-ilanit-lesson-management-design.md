...
| לינק תיאום | **לינק אישי לכל תלמיד** — אילנית לוחצת "שלח לינק לתיאום" במערכת → פופאפ (בחירת תלמיד קיים / הוספת חדש שם+טלפון) → לינק ייעודי עם טוקן מקושר ל-`student_id` נשלח בוואטסאפ לטלפון. **אין עמוד תיאום ציבורי כללי.** `/book/[token]` מזהה את התלמיד מהטוקן. |
...
| **booking_links** | `id, token_hash, student_id→, expires_at?, created_at` (לינק תיאום אישי שנשלח לתלמיד; הטוקן מזהה את התלמיד ב-`/book/[token]`) |
...
0. אילנית במערכת לוחצת **"שלח לינק לתיאום"** → פופאפ (בחירת תלמיד קיים / הוספת חדש שם+טלפון) → נוצר `booking_links` עם טוקן המקושר ל-`student_id` → וואטסאפ לתלמיד עם הלינק האישי `/book/[token]`.
1. התלמיד פותח את **הלינק האישי** `/book/[token]` (מזוהה מהטוקן — אין צורך למלא פרטים) → בוחר יום → רואה סלוטים פנויים → בוחר (מייל אופציונלי להוספה לצורך הזמנת יומן).
