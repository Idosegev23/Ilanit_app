...
/*
  OptionWheel — curved scroll/drag selector (React Bits, adapted for Ilanit v4).

  Two deliberate changes from upstream:
    1. 'use client' — App Router.
    2. A new `onCommit` prop that separates HIGHLIGHTING from ACTIVATING.
       `onChange` fires on every scroll tick; if that navigated, scrolling from
       the first item to the last would fire seven navigations. So `onChange`
       only reports the highlight, and `onCommit` fires on an explicit
       activation: clicking the already-centered item, or pressing Enter/Space.

  Styling (`.option-wheel*`) lives in app/globals.css so there is a single CSS
  entry point and no cascade race against Tailwind utilities.
*/
...
export interface OptionWheelProps {
  ...
  /** Fires as the wheel settles on a new option — HIGHLIGHT only. */
  onChange?: (index: number, item: string) => void;
  /** Fires on explicit activation: click the centered item, or Enter/Space. */
...
const OptionWheel = ({
  ...
  /** Explicit activation — the only path that should navigate. */
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent<HTMLDivElement>) => {
      ...
      if (!dragMovedRef.current && Math.abs(dy) > 4) {
        ...
        // items and select/activate them.
  ...
  const handleItemClick = useCallback(
    (index: number) => {
      ...
      const cfg = cfgRef.current;
      // Tapping the option that is already centered ACTIVATES it. Tapping any
      // other option just brings it to the middle.
